...
try {
    ...
    $newUserId = isset($data['user_id']) ? intval($data['user_id']) : intval($currentBooking['user_id']);
    $newBoatId = isset($data['boat_id']) ? intval($data['boat_id']) : intval($currentBooking['boat_id']);
    $newStartTime = isset($data['start_time']) ? $data['start_time'] : $currentBooking['start_time'];
    $newEndTime = isset($data['end_time']) ? $data['end_time'] : $currentBooking['end_time'];
    $newBookingDate = isset($data['booking_date']) ? $data['booking_date'] : $currentBooking['booking_date'];
    $newStatus = isset($data['status']) ? $data['status'] : $currentBooking['status'];
    ...
    $timeChanged = ($newBoatId !== intval($currentBooking['boat_id']))
        || ($newBookingDate !== $currentBooking['booking_date'])
        || ($newStartTime !== $currentBooking['start_time'])
        || ($newEndTime !== $currentBooking['end_time']);
    ...
    if ($shouldCheckAvailability && $timeChanged) {
        ...
        $checkStmt->execute([$newBoatId, $newBookingDate, $bookingId, $newStartTime, $newEndTime, $newStartTime, $newEndTime]);
    ...
    $amount = isset($data['amount']) ? floatval($data['amount']) : floatval($currentBooking['amount']);

    if (!isset($data['amount']) && ($itemsProvided || $newBoatId !== intval($currentBooking['boat_id']))) {
        if (!$itemsProvided) {
            $existingItemsStmt = $conn->prepare("SELECT quantity, price, price_discount FROM booking_items WHERE booking_id = ?");
            $existingItemsStmt->execute([$bookingId]);
