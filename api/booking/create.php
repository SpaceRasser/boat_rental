...
try {
    ...
    // Проверка существования owner_id в таблице owners
    $ownerCheck = $conn->prepare("SELECT id_owner FROM owners WHERE id_owner = ?");
    $ownerCheck->execute([$owner_id]);
    if (!$ownerCheck->fetch()) {
        sendError('Владелец с указанным ID не найден в системе');
    }
    
    // Проверка существования user_id в таблице users
    $userCheck = $conn->prepare("SELECT id_user FROM users WHERE id_user = ?");
    $userCheck->execute([$user_id]);
    if (!$userCheck->fetch()) {
        sendError('Пользователь с указанным ID не найден в системе');
    }
